...
use clap::Parser;
use aes_gcm::{
    aead::{AeadCore, AeadMutInPlace, KeyInit, OsRng}, Aes256Gcm
};
...
fn encrypt_aes(shellcode: &mut Vec<u8>) -> EncArgs{
    ...
    let a = nonce.len();
    cipher.encrypt_in_place(&nonce, b"", shellcode).expect("Could not encrypt shellcode");
    EncArgs {
        key: key.as_slice().try_into().expect("Key did not fit"),
        nonce: nonce.as_slice().try_into().expect("Nonce did not fit"),
    }
    //cipher.encrypt(&nonce, shellcode);
    //cipher.decrypt_in_place(&nonce, b"", shellcode);
}
...
fn main() -> std::io::Result<()> {
    ...
    if shellcode.len() > 0 {
        ...
        let enc_args = if args.encryption.is_some() {
            if args.encryption.unwrap() == "aes" {
                encrypt_aes(&mut shellcode)
        ...
        let content_shellcode_template = fs::read_to_string(&resource_path).expect("Could not read shellcode_template.rs");
        resource_path.pop();
